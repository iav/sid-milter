...
#ifdef WIN32
	if (_fstat(fd, &stat_buf) != 0)
	{
		errno = EINVAL;
		return -1;
	}

	/* Sockets and pipes don't have _S_IWRITE or _S_IREAD flags. */
	if ((stat_buf.st_mode & (_S_IFSOCK|_S_IFIFO)) == 0)
	{
		...
		if ((oflags & (O_RDONLY|O_RDWR)) != 0 &&
		    !(stat_buf.st_mode & _S_IREAD))
		{
			errno = EACCES;
			return -1;
		}
